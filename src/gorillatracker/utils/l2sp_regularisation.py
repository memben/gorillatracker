...
def strip_prefix(state_dict: StateDictType) -> StateDictType:
    """Removes the `model.` prefix from the layer names in a PyTorch Lightning checkpoint.

    This function is specifically designed to handle checkpoints generated by PyTorch Lightning,
    which prepend `model.` to the layer names.
    Args:
        state_dict (dict): The model's state_dict containing the layer parameters.

    Returns:
        dict: The modified state_dict with the `model.` prefix removed.
    """

    new_state_dict = collections.OrderedDict()
    for key, value in state_dict.items():
        if key.startswith("model."):
            new_key = key[6:]  # Strip the 'model.' prefix.
...
def get_keys_to_decay(model: nn.Module) -> List[str]:
...
class L2(nn.Module):
    """L2 regularization
    this is already builtin in most optimizers.
    """
    ...
    def forward(self, model: nn.Module) -> float:
        ...
        l2_loss = 0.0  # torch.tensor(0., requires_grad=True)
        for key, param in model.named_parameters():
            if key in self.keys:
                l2_loss += param.pow(2).sum() * 0.5  # type: ignore
...
class L2_SP(nn.Module):
    ...
    def __init__(self, model: nn.Module, path_to_pretrained_weights: str, alpha: float, beta: float) -> None:
        #
    ...
    @staticmethod
    def dots_to_underscores(key: str) -> str:
        return key.replace(".", "_")

    def get_keys(self, model: nn.Module, pretrained_state: StateDictType) -> Tuple[List[str], List[str]]:
...
def get_regularization_loss(model: nn.Module, args: Dict[str, Any]) -> Union[L2, L2_SP]:
    regularization_criterion: Union[L2, L2_SP]
    if args["regularization_style"] == "l2":
        log.info("Regularization: L2. alpha: {} ".format(args["weight_decay"]))
        regularization_criterion = L2(model, args["weight_decay"])
    elif args["regularization_style"] == "l2_sp":
        log.info(
            "Regularization: L2_SP. Pretrained file: {} alpha: {} beta: {}".format(
                args["pretrained_model"], args["weight_decay"], args["regularization_beta"]
            )
        )
        regularization_criterion = L2_SP(
            model, args["pretrained_model"], args["weight_decay"], args["regularization_beta"]
        )
    else:
...
class ClassificationLoss(nn.Module):
    ...
    def __init__(self, data_criterion: nn.Module, regularization_criterion: nn.Module) -> None:
        super().__init__()
        self.data_criterion = data_criterion
        self.regularization_criterion = regularization_criterion

    def forward(self, outputs: Any, label: Any, model: nn.Module) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
